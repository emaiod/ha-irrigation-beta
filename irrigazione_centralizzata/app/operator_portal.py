...
def current_user(request: Request) -> dict[str, Any] | None:
    ...
    try:
        ...
        with db() as conn:
            row = conn.execute(
                "SELECT id, display_name, username, enabled FROM operator_users WHERE id=? AND username=?",
...
def audit(username: str, action: str, detail: str = "") -> None:
    with db() as conn:
        conn.execute(
            "INSERT INTO operator_audit(created_at, username, action, detail) VALUES(?,?,?,?)",
...
async def admin_request(method: str, path: str) -> Any:
    async with httpx.AsyncClient(timeout=20) as client:
        response = await client.request(method, f"{ADMIN_API}/{path.lstrip('/')}")
...
async def operator_page(request: Request):
    return templates.TemplateResponse(
        request=request,
        name="operator.html",
        context={"user": current_user(request), "login_error": request.query_params.get("error") == "1"},
    )
...
async def login(username: str = Form(...), password: str = Form(...)):
    with db() as conn:
        row = conn.execute(
            "SELECT * FROM operator_users WHERE username=? COLLATE NOCASE",
            (username.strip(),),
        ).fetchone()
        ...
        conn.execute(
            "UPDATE operator_users SET last_login=? WHERE id=?",
